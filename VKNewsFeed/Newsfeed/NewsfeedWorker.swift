        ...
        fetcher.getFeed(nextBatchFrom: newFromInProcess) { [weak self] feed in
            ...
            if self?.feedResponse == nil {
                ...
                if let oldProfiles = self?.feedResponse?.profiles {
                    let oldProfilesFilterd = oldProfiles.filter { oldProfile -> Bool in
                        !feed.profiles.contains(where: { $0.id == oldProfile.id })
                    }
                    profiles.append(contentsOf: oldProfilesFilterd)
                }
                self?.feedResponse?.profiles = profiles
                
                var groups = feed.groups
                if let oldGroups = self?.feedResponse?.groups {
                    let oldGroupsFilterd = oldGroups.filter { oldGroups -> Bool in
                        !feed.groups.contains(where: { $0.id == oldGroups.id })
                    }
                    groups.append(contentsOf: oldGroupsFilterd)
                }
